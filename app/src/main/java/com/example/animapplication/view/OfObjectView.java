package com.example.animapplication.view;

import android.content.Context;
import android.graphics.Canvas;
import android.graphics.Color;
import android.graphics.Paint;
import android.graphics.PointF;
import android.support.annotation.Nullable;
import android.util.AttributeSet;
import android.view.View;

import com.example.animapplication.utils.Utils;

/**
 * Created by xwxwaa on 2019/5/23.
 */

public class OfObjectView extends View {
    public static final float RADIUS = Utils.dpToPixel(40);

    Paint paint = new Paint(Paint.ANTI_ALIAS_FLAG);

    PointF position = new PointF();

    public OfObjectView(Context context) {
        super(context);
    }

    public OfObjectView(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
    }

    public OfObjectView(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
    }

    {
        paint.setColor(Color.parseColor("#ff0000"));
    }

    public PointF getPosition() {
        return position;
    }

    public void setPosition(PointF position) {
        if (position != null) {
            this.position.set(position);
            invalidate();
        }
    }

    @Override
    public void onDraw(Canvas canvas) {
        super.onDraw(canvas);

        float innerPaddingLeft = RADIUS * 1;
        float innterPaddingRight = RADIUS * 1;
        float innterPaddingTop = RADIUS * 1;
        float innterPaddingBottom = RADIUS * 1;
        float width = getWidth() - innerPaddingLeft - innterPaddingRight ;
        float height = getHeight() - innterPaddingTop - innterPaddingBottom ;
        canvas.drawCircle( RADIUS + width * position.x,  RADIUS + height * position.y, RADIUS, paint);

    }
}
